<div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 text-sm font-medium text-gray-800">{{ $po->invoice_number }}</td>
                    <td class="px-4 py-3 text-sm text-gray-500">{{ $po->supplier?->name ?? '-' }}</td>
        ...
        <div class="px-4 py-3 border-t border-gray-200">{{ $purchases->links() }}</div>
    </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 overflow-y-auto">
        <div class="bg-white rounded-xl shadow-2xl p-6 max-w-2xl w-full mx-4 my-8">
            ...
            <div class="mb-4">
                ...
                <select wire:model="supplier_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required>
            ...
            <div class="mb-4">
                ...
                <div class="mt-2 border border-gray-200 rounded-lg max-h-40 overflow-y-auto">
                    @foreach($searchResults as $p)
                    <button wire:click="addItem({{ $p['id'] }})" class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 border-b border-gray-100">
            ...
            <div class="space-y-2 mb-4">
                ...
                <div class="flex items-center gap-2 bg-gray-50 rounded-lg p-2">
                    ...
                    <input type="number" value="{{ $item['quantity'] }}" wire:change="updateItemQty({{ $index }}, $event.target.value)"
                           class="w-16 text-center border border-gray-300 rounded text-sm py-1" min="1">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white rounded-xl shadow-2xl p-6 max-w-sm w-full mx-4">
            ...
            <p class="text-sm text-gray-500 mb-4">Stok barang akan ditambahkan secara otomatis. Lanjutkan?</p>
            <div class="flex gap-2 justify-end">
                <button wire:click="$set('showReceiveModal', false)" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 text-sm">Batal</button>
