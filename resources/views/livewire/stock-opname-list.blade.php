<div>
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4 text-sm text-amber-800">
        ...
        <span class="text-amber-600">Stok akan langsung diperbarui sesuai stok aktual.</span>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">No. Opname</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Produk</th>
                    <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Stok Sistem</th>
                    <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Stok Aktual</th>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 cursor-pointer" wire:click="viewDetail({{ $opname->id }})">
                    <td class="px-4 py-3 text-sm font-medium text-gray-800">{{ $opname->opname_number }}</td>
                    <td class="px-4 py-3 text-sm text-gray-700">{{ $opname->product?->name ?? '-' }}</td>
                    ...
                    <td class="px-4 py-3 text-sm text-right font-semibold {{ $opname->difference > 0 ? 'text-emerald-600' : ($opname->difference < 0 ? 'text-red-600' : 'text-gray-500') }}">
                        {{ $opname->difference > 0 ? '+' : '' }}{{ number_format($opname->difference) }}
                    </td>
                    <td class="px-4 py-3 text-center">
                        <span class="text-xs px-2 py-1 rounded-full 
                            @if($opname->type === 'surplus') bg-emerald-100 text-emerald-700
                            @elseif($opname->type === 'shortage') bg-red-100 text-red-700
                            @else bg-gray-100 text-gray-700 @endif">
                            {{ ucfirst($opname->type) }}
                        </span>
                    </td>
                    <td class="px-4 py-3 text-sm text-gray-500">{{ $opname->user?->name ?? '-' }}</td>
        ...
        <div class="px-4 py-3 border-t border-gray-200">{{ $opnames->links() }}</div>
    </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white rounded-xl shadow-2xl p-6 max-w-lg w-full mx-4">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-semibold text-gray-800">Stok Opname Baru</h3>
            ...
            <form wire:submit="save" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="mt-1 border border-gray-200 rounded-lg max-h-40 overflow-y-auto shadow">
                        @foreach($searchResults as $p)
                        <button type="button" wire:click="selectProduct({{ $p['id'] }})" 
                                class="w-full text-left px-3 py-2 text-sm hover:bg-emerald-50 border-b border-gray-100">
                            <span class="font-medium">{{ $p['name'] }}</span>
                            <span class="text-gray-400 ml-2">SKU: {{ $p['sku'] ?? '-' }}</span>
                            <span class="text-gray-500 ml-2">Stok: {{ $p['stock'] }}</span>
                        </button>
                        @endforeach
                    </div>
                    @endif
                    @if($selectedProductName)
                    <div class="mt-2 flex items-center bg-emerald-50 rounded-lg px-3 py-2 text-sm text-emerald-700">
                ...
                <div class="bg-gray-50 rounded-lg p-3">
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600">Stok Sistem Saat Ini:</span>
                        <span class="font-bold text-gray-800">{{ number_format($currentStock) }}</span>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="number" wire:model.live="actual_stock" 
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" min="0" required>
                    @error('actual_stock') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                </div>

                <!-- Difference Preview -->
                @if($actual_stock !== null)
                <div class="rounded-lg p-3 {{ $difference > 0 ? 'bg-emerald-50' : ($difference < 0 ? 'bg-red-50' : 'bg-gray-50') }}">
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600">Selisih:</span>
                        <span class="font-bold {{ $difference > 0 ? 'text-emerald-600' : ($difference < 0 ? 'text-red-600' : 'text-gray-500') }}">
                ...
                <div>
                    ...
                    <textarea wire:model="notes" rows="2" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" 
                ...
                <div class="flex gap-2 justify-end pt-2 border-t border-gray-200">
                    <button type="button" wire:click="$set('showForm', false)" 
                            class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 text-sm">Batal</button>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full mx-4">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-semibold text-gray-800">Detail Stok Opname</h3>
            ...
            <div class="space-y-3">
                ...
                <div class="border-t border-gray-200 pt-3">
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">Stok Sistem:</span>
                        <span>{{ number_format($showDetail->system_stock) }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">Stok Aktual:</span>
                        <span class="font-medium">{{ number_format($showDetail->actual_stock) }}</span>
                    </div>
                    <div class="flex justify-between text-sm font-bold mt-1 pt-1 border-t border-gray-100">
                        <span>Selisih:</span>
                        <span class="{{ $showDetail->difference > 0 ? 'text-emerald-600' : ($showDetail->difference < 0 ? 'text-red-600' : 'text-gray-500') }}">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    <span class="px-2 py-0.5 text-xs rounded-full 
                        @if($showDetail->type === 'surplus') bg-emerald-100 text-emerald-700
                        @elseif($showDetail->type === 'shortage') bg-red-100 text-red-700
                        @else bg-gray-100 text-gray-700 @endif">
                ...
                <div>
                    ...
                    <p class="text-sm bg-gray-50 rounded-lg p-2">{{ $showDetail->notes }}</p>
                </div>
                @endif
                <div class="flex justify-between text-sm">
            ...
            <button wire:click="$set('showDetail', null)" 
                    class="w-full mt-4 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 text-sm">Tutup</button>
        </div>
    </div>
    @endif
</div>
