<div>
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                ...
                <div class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2 max-h-64 overflow-y-auto">
                    ...
                    <button wire:click="addToCart({{ $product['id'] }})" 
                            class="flex items-center justify-between p-3 rounded-lg border border-gray-200 hover:border-emerald-400 hover:bg-emerald-50 transition-colors text-left">
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-800 truncate">{{ $product['name'] }}</p>
                            <p class="text-xs text-gray-500">Stok: {{ $product['stock'] }} {{ $product['unit']['name'] ?? 'Unit' }}</p>
                        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                <h4 class="text-sm font-medium text-gray-600 mb-3">Kategori Cepat</h4>
                <div class="flex flex-wrap gap-2">
                    @foreach(\App\Models\Category::where('is_active', true)->get() as $category)
                    <button wire:click="$set('search', '{{ $category->name }}')" 
                            class="px-3 py-1.5 text-xs font-medium bg-gray-100 text-gray-700 rounded-full hover:bg-emerald-100 hover:text-emerald-700 transition-colors">
                        {{ $category->name }}
                    </button>
                    @endforeach
                </div>
            ...
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                <div class="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full mx-4">
                    ...
                    <div class="flex gap-2 mt-4">
                        ...
                        <button wire:click="newTransaction" class="flex-1 px-4 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors text-sm font-medium">
                            Transaksi Baru
                        </button>
                    </div>
                </div>
        ...
        <!-- Right: Cart -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex flex-col" style="height: calc(100vh - 8rem);">
                ...
                <div class="bg-gray-50 rounded-lg p-3">
                    <div class="flex items-center justify-between">
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-500">Rp {{ number_format($item['selling_price'], 0, ',', '.') }}/{{ $item['unit'] }}</p>
                        </div>
                    ...
                    <div class="flex items-center justify-between mt-2">
                        <div class="flex items-center space-x-2">
                            <button wire:click="updateQuantity({{ $index }}, {{ $item['quantity'] - 1 }})" 
                                    class="w-7 h-7 bg-gray-200 rounded-full flex items-center justify-center text-gray-600 hover:bg-gray-300 text-sm">-</button>
                            <input type="number" value="{{ $item['quantity'] }}" 
                                   wire:change="updateQuantity({{ $index }}, $event.target.value)"
                                   class="w-14 text-center border border-gray-300 rounded-lg text-sm py-1" min="1" max="{{ $item['stock'] }}">
                            <button wire:click="updateQuantity({{ $index }}, {{ $item['quantity'] + 1 }})" 
                                    class="w-7 h-7 bg-gray-200 rounded-full flex items-center justify-center text-gray-600 hover:bg-gray-300 text-sm">+</button>
                        </div>
            ...
            <div class="border-t border-gray-200 pt-4 space-y-2">
                ...
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">Diskon</span>
                    <input type="number" wire:model.live="discount" class="w-28 text-right border border-gray-300 rounded text-sm py-1" placeholder="0">
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">Pajak (PPN)</span>
                    <input type="number" wire:model.live="tax" class="w-28 text-right border border-gray-300 rounded text-sm py-1" placeholder="0">
                </div>
        ...
        <div class="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full mx-4">
            ...
            <div class="max-h-40 overflow-y-auto space-y-1 mb-4">
                ...
                <button wire:click="selectCustomer({{ $customer['id'] }})" 
                        class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 rounded-lg">
            ...
            <div class="border-t border-gray-200 pt-4">
                <h4 class="text-sm font-medium text-gray-700 mb-2">Pelanggan Baru</h4>
                <input type="text" wire:model="customer_name" placeholder="Nama" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mb-2">
                <input type="text" wire:model="customer_phone" placeholder="Telepon" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mb-3">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full mx-4">
            ...
            <div class="space-y-3 mb-4">
                ...
                <div>
                    <label class="text-sm text-gray-600">Jumlah Dibayar</label>
                    <input type="number" wire:model.live="paid_amount" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mt-1">
                </div>
                ...
                <div>
                    ...
                    <select wire:model="payment_status" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mt-1">
                ...
                <div>
                    ...
                    <textarea wire:model="notes" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mt-1" rows="2"></textarea>
                </div>
            </div>

            <div class="flex gap-2">
                <button wire:click="$toggle('showPaymentModal')" class="flex-1 px-4 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 text-sm">
                    Batal
                </button>
