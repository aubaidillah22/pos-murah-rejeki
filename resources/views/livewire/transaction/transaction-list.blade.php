<div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div class="flex-1 flex flex-wrap items-center gap-2">
            ...
            <select wire:model.live="paymentMethod" class="border border-gray-300 rounded-lg px-3 py-2 text-sm">
            ...
            <select wire:model.live="paymentStatus" class="border border-gray-300 rounded-lg px-3 py-2 text-sm">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 cursor-pointer" wire:click="viewDetail({{ $t->id }})">
                    <td class="px-4 py-3 text-sm font-medium text-gray-800">{{ $t->invoice_number }}</td>
                    <td class="px-4 py-3 text-sm text-gray-500">{{ $t->transaction_date->format('d/m/Y H:i') }}</td>
                    <td class="px-4 py-3 text-sm text-gray-500">{{ $t->customer?->name ?? 'Umum' }}</td>
                    <td class="px-4 py-3 text-sm text-gray-500">{{ $t->user?->name }}</td>
                    <td class="px-4 py-3 text-sm font-medium text-right text-emerald-600">Rp {{ number_format($t->grand_total, 0, ',', '.') }}</td>
                    <td class="px-4 py-3 text-center">
                        <span class="text-xs px-2 py-1 rounded-full {{ $t->payment_status === 'paid' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                            {{ $t->payment_status === 'paid' ? 'Lunas' : 'Piutang' }}
                        </span>
                    </td>
                    <td class="px-4 py-3 text-right text-xs text-gray-400">{{ ucfirst($t->payment_method) }}</td>
                </tr>
                @empty
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white rounded-xl shadow-2xl p-6 max-w-lg w-full mx-4">
            ...
            <div class="border-t border-gray-200 my-4 pt-4">
                <h4 class="text-sm font-medium text-gray-700 mb-2">Item</h4>
                @foreach($showDetail->details as $d)
                <div class="flex justify-between text-sm py-1">
                    <span class="text-gray-600">{{ $d->product?->name }} x{{ $d->quantity }}</span>
            ...
            <div class="border-t border-gray-200 pt-3 space-y-1">
                ...
                <div class="flex justify-between text-sm"><span class="text-gray-500">Diskon</span><span class="text-red-500">-Rp {{ number_format($showDetail->discount, 0, ',', '.') }}</span></div>
                @endif
                @if($showDetail->tax > 0)
                <div class="flex justify-between text-sm"><span class="text-gray-500">Pajak</span><span>Rp {{ number_format($showDetail->tax, 0, ',', '.') }}</span></div>
                @endif
            ...
            <div class="flex gap-2 mt-4">
                <button wire:click="$set('showDetail', null)" class="flex-1 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 text-sm">Tutup</button>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full mx-4">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-semibold text-red-600">Void Transaksi</h3>
            ...
            <form wire:submit="voidTransaction" class="space-y-4">
                ...
                <div class="flex gap-2 justify-end">
                    <button type="button" wire:click="$set('showVoidModal', false)" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 text-sm">Batal</button>
