<div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 {{ !$outlet->is_active ? 'opacity-50' : '' }}">
                    <td class="px-4 py-3 text-sm font-medium text-gray-800">{{ $outlet->name }}</td>
                    <td class="px-4 py-3 text-sm text-gray-500">{{ $outlet->address ?? '-' }}</td>
                    <td class="px-4 py-3 text-sm text-gray-500">{{ $outlet->phone ?? '-' }}</td>
                    <td class="px-4 py-3 text-sm text-gray-500">{{ $outlet->email ?? '-' }}</td>
                ...
                @empty
                <tr><td colspan="6" class="px-4 py-8 text-center text-gray-400">Belum ada outlet</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white rounded-xl shadow-2xl p-6 max-w-sm w-full mx-4">
            <h3 class="text-lg font-semibold text-gray-800 mb-2">Hapus Outlet?</h3>
